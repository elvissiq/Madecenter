...
Local cNomCli       := Posicione("SA1",1,xFILIAL("SA1")+ALLTRIM(SL1->L1_CLIENTE),"A1_NREDUZ") //Nome do Cliente
...
Local aArea 		:= GetArea()	// Guarda area atual
Local sTexto		:= ""			// Texto do comprovante a ser impresso
Local nLarCup 		:= 40			// Largura do cupom de acordo com cadastro de estacao
Local cPicValor     := PesqPict("SL1","L1_VLRTOT")//Picture de valor
Local nMaxChar 		:= 47 
Local aFieldSM0 	:= { ;
...
Local cCgcEnt       := aSM0Data[7,2] // CNPJ 
Local cInsEnt       := aSM0Data[8,2] // Inscrição Estadual
Local cNomCli       := Posicione("SA1",1,xFILIAL("SA1")+ALLTRIM(SL1->L1_CLIENTE),"A1_NOME") //Nome do Cliente
Local cCGCCli       := Posicione("SA1",1,xFILIAL("SA1")+ALLTRIM(SL1->L1_CLIENTE),"A1_CGC") //CPF/CNPJ do Cliente
Local cTpCli        := Posicione("SA1",1,xFILIAL("SA1")+ALLTRIM(SL1->L1_CLIENTE),"A1_TIPO") //Tipo do Cliente
Local cNomVen       := Posicione("SA3",1,xFilial("SA3")+SL1->L1_VEND,"A3_NOME")  // Nome do Vendedor
Local cNomOpe       := Posicione("SA6",1,xFilial("SA6")+SL1->L1_OPERADO,"A6_NOME")  // Nome do Operador
Local cMsgCart      := SuperGetMV("MV_XMSGCART",.F.,"")
...
EndIf
If cTpCli == 'F'
	cCGCCli := Alltrim(Transform(cCGCCli, "@R 999.999.999-99"))
ElseIF cTpCli == 'J'
	cCGCCli := Alltrim(Transform(cCGCCli, "@R 99.999.999/9999-99"))
EndIF
...
If Len(aNCCs) > 0 .AND. nVlrCred > 0
	...
	sTexto:= sTexto + Replicate("-", nMaxChar)						   + Chr(13)+ Chr(10)
	sTexto:= sTexto + '<b><ce>C O M P R O V A N T E   C A R T E I R A</ce></b>'		   + Chr(13)+ Chr(10)
	sTexto:= sTexto + Replicate("-", nMaxChar)						   + Chr(13)+ Chr(10)

	sTexto:= sTexto + Replicate(" ", nMaxChar)						   + Chr(13)+ Chr(10)	
	sTexto += CHR(10) + PADR("Valor utilizado na venda:",nLarCup-14) + PADL("R$ " +AllTrim(Transform(nVlrCred,cPicValor)),14) + CHR(10)
	sTexto:= sTexto + Replicate(" ", nMaxChar)						   + Chr(13)+ Chr(10)
	sTexto:= sTexto + Replicate("-", nMaxChar)						   + Chr(13)+ Chr(10)
	
	sTexto:= sTexto + Replicate(" ", nMaxChar)						   + Chr(13)+ Chr(10)
	sTexto := sTexto + '<ce>' + cMsgCart + '</ce>' + Chr(13) + Chr(10)
	sTexto:= sTexto + Replicate(" ", nMaxChar)						   + Chr(13)+ Chr(10)

	sTexto := sTexto + '<ce>'+ Replicate("_", nMaxChar-7) +'</ce> '  + Chr(13) + Chr(10)
	sTexto:= sTexto + Replicate(" ", nMaxChar)						   + Chr(13)+ Chr(10)
	sTexto := sTexto + '<b>Cliente:</b> ' +  Alltrim(cCGCCli) +" - "+ PADR(Alltrim(cNomCli),nMaxChar)    + Chr(13) + Chr(10)
	sTexto := sTexto + Replicate("-", nMaxChar)						     + Chr(13) + Chr(10) 
	sTexto := sTexto + '<b>Data:</b> ' + DtoC(dDatabase) + ' <b>Hora: </b>' +Time() + Chr(13) + Chr(10)
	sTexto := sTexto + '<b>Vendedor:</b> ' + Alltrim(SL1->L1_VEND)+' - ' +  Alltrim(cNomVen) + Chr(13) + Chr(10)
	sTexto := sTexto + '<b>Caixa:</b> ' + Alltrim(SL1->L1_ESTACAO)+'<b> Operador: </b>' + Alltrim(SL1->L1_OPERADO)+' - ' +  Alltrim(cNomOpe) + Chr(13) + Chr(10)
